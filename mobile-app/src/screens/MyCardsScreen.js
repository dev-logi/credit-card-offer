...
import { getCardById, AVAILABLE_CARDS } from '../data/availableCards';
...
export default function MyCardsScreen({ navigation }) {
  ...
  const getCardDetails = (cardName) => {
    ...
    for (const availableCard of Object.values(AVAILABLE_CARDS)) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <FAB
        ...
        onPress={async () => {
          const customerId = await AsyncStorage.getItem('customerId');
          navigation.navigate('SelectCards', { customerId, isFirstTime: false });
        }}
      />
    </SafeAreaView>
  );
